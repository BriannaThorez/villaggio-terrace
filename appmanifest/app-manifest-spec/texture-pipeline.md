...
flowchart TD
  subgraph DISK["💾 Disk: assets/textures/<name>/"]
    PNG_4K["*_4k.png (Master Sources: diff, arm, nor, disp)"]
    VARIANTS["*_512.png, *_1k.png, *_2k.png\n(Auto-generated by build script)"]
    PNG_4K --> VARIANTS
  end

  subgraph BUILD["🛠️ Build Time — generate_texture_variants.mjs"]
    SHARP["sharp\nDownsamples 4K master to 512, 1k, 2k\nSkips if mtime unchanged"]
  end

  subgraph REGISTRY_BUILD["🗃️ Registry — materials.ts"]
    GLOB["import.meta.glob()\nScans all .png variants\nEager — zero runtime cost"]
    REGISTRY["ASSET_REGISTRY\nMaps type + resolution to URL\nNormalized: nor_gl → nor"]
    GLOB --> REGISTRY
  end
  ...
  subgraph SETTINGS["⚙️ Settings Panel"]
    SLIDER["Quality Slider (Low...Ultra)\nTriggers: textureLODHandler.clearCache()\ntriggerQualityShiftPrewarm()"]
  end

  SETTINGS -->|"clear + shift"| MEMCACHE
  SETTINGS -->|"re-warm"| INIT

  MEMCACHE -->|"CACHE HIT → tier bundle"| R4
  MEMCACHE -->|"CACHE MISS → placeholder + promise"| PLACEHOLDER
  PLACEHOLDER --> R4
  ACTIVELOADS --> SW1

  subgraph GPU["🖥️ GPU"]
    VRAM["VRAM\nResolution-tiered textures\n+ Compiled Shaders"]
  end
  ...
  subgraph VISUAL["👁️ Visual Output"]
    ...
    FULL["✅ Selected Tier (512-4K)\nLive upgrade/downgrade"]
  end
...
1. Create `src/assets/textures/<texture_name>/`
2. Add master files with `_4k.png` suffix:
   - `*_diff_4k.png` (Albedo)
   - `*_arm_4k.png` (AO/Roughness/Metalness)
   - `*_nor_gl_4k.png` (Normal GL)
   - `*_disp_4k.png` (Displacement)
3. Run `npm run generate:textures` (or just `npm run dev`)
4. Build script automatically creates `_512`, `_1k`, and `_2k` variants
5. Auto-discovered by `ASSET_REGISTRY` at build time
6. **Manifest room** → add to `rooms[].metadata` in `roomMetadata.json`
7. **Hardcoded room** (Lobby/Structure/EmptyFloor) → also add to base seed in `preload.ts`
8. Verify: `[Program-Initialization Prewarmer] Injecting warm bundle → <name> (tier-bundle)`
