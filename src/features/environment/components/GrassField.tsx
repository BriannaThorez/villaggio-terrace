...
class SimplexNoise2D {
    ...
    noise2D(x: number, z: number): number {
        ...
        return (aa * (1 - u) + ba * u) * (1 - v) + (ab * (1 - u) + bb * u) * v - 0.5;
    }
}
...
function createGroundGeometry(width: number, segments = 32): THREE.BufferGeometry {
...
export const GrassField: React.FC<GrassFieldProps> = ({
    ...
    const cullData = useMemo(() => {
        ...
        const groundRooms = shapes.filter(s =>
    ...
    const baseGeom = useMemo(
        () => new THREE.PlaneGeometry(bladeWidth, bladeHeight, 1, joints).translate(0, bladeHeight / 2, 0),
    ...
    return (
        <group position={position}>
            {/* Instanced grass blades — single draw call (Shadows disabled for infinite performance) */}
            <mesh frustumCulled={false} renderOrder={renderOrder} castShadow={false} receiveShadow={false}>
                ...
                <grassMaterial ref={materialRef} bladeHeight={bladeHeight} toneMapped={false} />
            </mesh>
        </group>
    );
};
