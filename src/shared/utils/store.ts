...
export type ShapeType = 
  ...
  | 'residential' | 'commercial' | 'office' | 'utility' | 'lobby' | 'elevator';
...
export interface FlowchartState {
  ...
  activeTool: ShapeType | 'link' | 'select' | 'vertex' | 'residential' | 'commercial' | 'office' | 'utility' | 'lobby' | 'elevator';
  mode: 'studio' | 'viewer';
  ...
  addShape: (shape: Shape, force?: boolean, skipHistory?: boolean) => void;
...
export const useFlowchartStore = create<FlowchartState>((set, get) => {
  ...
  return {
    ...
    addShape: (shape, force = false, skipHistory = false) => {
      ...
      set((state) => {
        const safePos = force ? shape.position : findBestPosition(shape, state.shapes);
    ...
    setCameraRotation: (azimuth, polar) => set({ 
