...
const isInsideShape = (uv: THREE.Vector2 | undefined, shape: Shape) => {
  ...
  if (shape.type === 'box' || shape.type === 'text' || shape.type === 'cylinder' || shape.type === 'document' || 
      ['residential', 'commercial', 'office', 'utility', 'lobby', 'elevator'].includes(shape.type)) {
...
export const FlowchartNodes = () => {
  ...
  const getShapeType = (type: string) => {
    const mapping: Record<string, number> = {
      ...
      'residential': 0.0, 'commercial': 0.0, 'office': 0.0, 'utility': 0.0, 'lobby': 0.0, 'elevator': 0.0
    };
  ...
  useEffect(() => {
    ...
    shapes.forEach((shape, i) => {
      ...
      const isRoom = ['residential', 'commercial', 'office', 'utility', 'lobby', 'elevator'].includes(shape.type);
  ...
  const handleNodePointerDown = (e: any, id: string) => {
    ...
    // Start dragging
    const isDraggableTool = activeTool === 'select';
