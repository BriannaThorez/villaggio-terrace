...
- [x] **1.1** Create `src/features/population/` feature slice directory structure:
  - `types/`, `store/`, `lifecycle/`, `scheduler/`, `navigation/`, `components/`, `hooks/`
- [x] **1.2** Create `src/features/population/types/occupant.ts`
  - [x] Define `TenantLifecycleState` union
  - [x] Define `GuestLifecycleState` union
  - [x] Define `TenantEntity` interface
  - [x] Define `GuestEntity` interface
  - [x] Define `OccupantEntity` discriminated union
  - [x] Define `NavWaypoint` and `NavPath` types
- [x] **1.3** Create `src/features/population/store/populationStore.ts`
  - [x] `occupants: Record<string, OccupantEntity>`
  - [x] `selectedOccupantId: string | null`
  - [x] `addOccupant`, `removeOccupant`, `updateOccupantLifecycle`, `setSelectedOccupantId` actions
- [x] **1.4** Create `src/features/population/store/occupantPositions.ts`
  - [x] Define `OccupantTransform` interface
  - [x] Create `OccupantPositionStore` class (mirrors `SimPositionStore` pattern)
  - [x] Export singleton `occupantPositions`
- [x] **1.5** Create `src/features/population/store/roomOccupancyIndex.ts`
  - [x] Mutable `Map<roomId, Set<occupantId>>`
  - [x] `addOccupantToRoom`, `removeOccupantFromRoom`, `getOccupantsForRoom`, `clear` API
- [x] **1.6** Modify `src/features/tenancy/store/tenancyStore.ts`
  - [x] Add `occupantId?: string` to `TenantData`
  - [x] Add `roomCapacity?: number` and `slotOccupants: string[]` to `TenantData`
  - [x] Add `assignOccupantToRoom(roomId, occupantId)` action
  - [x] Add `removeOccupantFromRoom(roomId, occupantId)` action
- [x] **1.7** Run `npx tsc --noEmit` — confirm zero new errors
- [x] **1.8** Run `npx eslint src/features/population src/features/tenancy --ext .ts,.tsx`
- [x] ⏸️ **Phase 1 QA Checkpoint** — Pause for user review before proceeding to Phase 2
