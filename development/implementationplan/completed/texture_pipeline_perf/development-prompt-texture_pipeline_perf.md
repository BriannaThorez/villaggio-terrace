...
## User Prompt — Follow-Up (2026-04-20) — Structure/Empty Room Lag & Category Prewarming

> When placing structure/empty rooms I get significant lag — leading me to believe that lobby textures may be the only prewarmed textures, when structure and emptyroom are also both hard-coded and thus should be prewarmed as well.
> I get significant lag/delay when placing structure/empty rooms even sequentially and I'm uncertain why but it seems to generate delay events in the dev console.
> `[Violation] Handling of 'wheel' input event was delayed for 122 ms`
> When I mouse-hover over the build button for structure/empty room I get additional each time I hover:
> `[Violation] 'requestAnimationFrame' handler took <N>ms`
>
> Also lets shift the room build-menu prewarmer to begin prewarming when the user clicks the class(main build-toolbar) button:
> **E-HOVER-CATEGORY — Category-Level Warming**: When the user clicks a category tab (e.g. "Apartment"), pre-warm all rooms in that category simultaneously in a background batch — not one-by-one on individual room hover. Ensure prewarming logic uses the incremental system.
> Batch or sequential? Should it prioritize the selected room in case the user navigates quickly?

---

## AI Analysis — Follow-Up

### Diagnosed Root Cause — Structure/EmptyRoom Lag

**Finding 1: GPU shader stall — `getEmptyFloorMaterials()` was absent from the compile queue.**
`EmptyRoom.tsx` calls `parseRoomMaterial({ wallTexture: "concrete_wall_1" })` directly at mount time via `useMemo`. `parseRoomMaterial` triggers `createRoomSurfaceMaterial` → `getBundleProgressiveSync` → placeholder bundle with a 50ms deferred promise swap. While `concrete_wall_1` was in the *texture* preload, the `getEmptyFloorMaterials()` **material variants** were never added to the `materialQueue` passed to `renderer.compile()`. This meant the GPU had never seen those specific shader configurations before first placement — causing a shader compilation stall at placement time.
**Fix**: Added `...getEmptyFloorMaterials()` to the GPU `materialQueue` in `preload.ts` line ~178.

**Finding 2: EmptyRoom uses `concrete_wall_1` for its floor surface** (not `concrete_floor_1`). The `HARDCODED_ROOM_TEXTURES` comment was updated to reflect this accurately.

**Finding 3: RAF/wheel violations on hover** — The `onPointerEnter` on each room button fires `audioEngine.triggerMenuExpand()` + a Zustand state write + `warmForModule` synchronously. The RAF violation is not caused by the prewarmer itself (it's async), but by the cumulative cost of the audio engine call + React re-render + tooltip position recalculation happening inside the event handler. Category prewarming eliminates `warmForModule` as a concern by having textures ready before hover occurs.

### Architecture Decision — Batch vs Sequential

**Batch (`Promise.all`) is correct for category warming:**
- User has not declared intent about a specific room yet — all rooms in the category are equally likely
- Parallel fetches maximize throughput; browser handles concurrent HTTP requests natively (HTTP/2 multiplexing)
- Category has typically 2–6 rooms — batch overhead is negligible

**Priority promotion for selected room:**
- `useToolPreloader` already calls `textureLODHandler.promoteToForeground(txName)` on `activeModuleId` change
- If the batch load is still in-flight when the user clicks a room, `promoteToForeground` returns the same `activeLoads` promise — no duplicate fetch, just re-prioritized resolution order
- This is the correct layered architecture: category batch → selected room promotion

### New Hook: `useCategoryPreloader`
- Fires on `handleCategoryClick()` in `BuildToolbar.tsx` for categories with subTypes
- Resolves all texture names in the category using the same three-level fallback as `ResidentialRoom.tsx`
- `hasCachedBundle()` dedup gate: repeat clicks are O(1) no-ops
- Batch fetch via `getTextureBundle()` + `injectBundle()` confirms results into LOD cache
- `getBundleProgressiveSync()` called first to register in `activeLoads` for promotion compatibility
